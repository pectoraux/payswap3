"""Composite sealing for clearing-domain durable objects.

Every clearing-domain durable object is ``ObjectEnvelope + payload`` and
...
def build_domain_envelope(
    *,
    ...
    envelope = ObjectEnvelope(
        ...
        domain_id=domain_id,
        schema_version=CLEARING_SCHEMA_VERSION,
        protocol_version=CLEARING_PROTOCOL_VERSION,
        state=state,
        provenance=provenance,
...
def decode_composite(
    ...
    *,
    ...
    registry-listed obligation type or an internal ``clearing/...``
    type; claiming a registry-governed type foreign to this record kind
    fails closed with a dedicated message), protocol and schema
    versions, closed state vocabulary, payload container type.
    """
    ...
    if envelope.object_type != object_type:
        if str(envelope.object_type).startswith("payswap/") and not object_type.startswith(
            ...
            raise CoreValidationError(
                "clearing object_type must not claim a registry-governed "
    ...
    if envelope.protocol_version != CLEARING_PROTOCOL_VERSION:
        raise CoreValidationError(
            f"{object_type} requires protocol version "
            f"{CLEARING_PROTOCOL_VERSION}, got {envelope.protocol_version!r}"
        )
    if envelope.schema_version != CLEARING_SCHEMA_VERSION:
        raise CoreValidationError(
            f"{object_type} requires schema version "
            f"{CLEARING_SCHEMA_VERSION}, got {envelope.schema_version}"
        )
    try:
