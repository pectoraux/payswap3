"""Composite sealing for liquidity domain durable objects.

Every liquidity domain durable object is ``ObjectEnvelope + payload`` and
...
objects fail closed on the trusted deserialization path. No second hash
or error authority is introduced.
"""
...
from .contracts import LIQUIDITY_PROTOCOL_VERSION, LIQUIDITY_SCHEMA_VERSION
...
def build_domain_envelope(
    *,
    ...
    envelope = ObjectEnvelope(
        ...
        protocol_version=LIQUIDITY_PROTOCOL_VERSION,
        state=state,
        provenance=provenance,
...
def decode_composite(
    ...
    *,
    ...
    if envelope.protocol_version != LIQUIDITY_PROTOCOL_VERSION:
        raise CoreValidationError(
            f"{expected_object_type} requires protocol version "
            f"{LIQUIDITY_PROTOCOL_VERSION}, got {envelope.protocol_version!r}"
        )
    if envelope.schema_version != LIQUIDITY_SCHEMA_VERSION:
        raise CoreValidationError(
            f"{expected_object_type} requires schema version "
            f"{LIQUIDITY_SCHEMA_VERSION}, got {envelope.schema_version}"
        )
    try:
