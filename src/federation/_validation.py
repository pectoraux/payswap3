"""Strict fail-closed validation helpers for the federation domain.
...
def parse_utc_timestamp(name: str, value: str) -> datetime:
    ...
    Timestamps are declared data, never clock reads: the federation
    domain is deterministic and every instant is supplied explicitly.
    The canonical ``Z`` form is required (fail closed on other offsets)
    so ordering and freshness arithmetic are total, unambiguous and
    byte-stable.
    """
    require_text(name, value)
