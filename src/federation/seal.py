"""Composite sealing for federation-domain durable objects.

Every federation-domain durable object is ``ObjectEnvelope + payload``
and carries a domain seal: a SHA-256 digest computed with the single
...
def decode_composite(
    ...
    *,
    ...
    ``federation/...`` types only; a record claiming a
    registry-governed ``payswap/`` type fails closed with a dedicated
    message), protocol and schema versions, closed state vocabulary,
    payload container type (a non-empty mapping — every federation
    record kind carries substantive payload facts).
    """
    ...
    if envelope.object_type != object_type:
        if str(envelope.object_type).startswith("payswap/"):
            raise CoreValidationError(
                "federation object_type must not claim a registry-governed "
    ...
    if envelope.protocol_version != FEDERATION_PROTOCOL_VERSION:
        raise CoreValidationError(
            f"{object_type} requires protocol version "
            f"{FEDERATION_PROTOCOL_VERSION}, got {envelope.protocol_version!r}"
        )
    if envelope.schema_version != FEDERATION_SCHEMA_VERSION:
        raise CoreValidationError(
            f"{object_type} requires schema version "
            f"{FEDERATION_SCHEMA_VERSION}, got {envelope.schema_version!r}"
        )
    try:
    ...
    if not isinstance(payload, Mapping) or not payload:
        raise CoreValidationError(f"{object_type} payload must be a non-empty object")
...
def decode_composite_json(
    ...
    *,
    ...
    Returns the full canonical composite mapping (envelope, payload and
    integrity hash) for byte-stable comparisons.
    """
    decoded = loads_canonical(value)
    if not isinstance(decoded, dict):
        raise CoreValidationError(f"{object_type} JSON must decode to an object")
    decode_composite(decoded, object_type=object_type, state_type=state_type)
    return decoded
