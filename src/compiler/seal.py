"""Composite sealing for compiler domain durable objects.

Every compiler durable object is ``ObjectEnvelope + payload`` and carries a
domain seal: a SHA-256 digest computed with the single canonical hash
authority (``src.core.serialization.canonical_sha256``) over the sealed
envelope plus the canonical payload. This mirrors the intent domain's
remediated pattern (W032: trusted deserialization verifies integrity and
rejects unsealed or tampered envelopes) at the composite layer, so tampered
or spliced composite objects fail closed. No second hash authority is
introduced: ``canonical_sha256`` from ``src.core`` is the only digest
function used.
"""
...
from ._validation import require_utc_timestamp, strict_fields
from .contracts import COMPILER_PROTOCOL_VERSION, COMPILER_SCHEMA_VERSION
...
def decode_composite(
    ...
    *,
    ...
    Order of checks: canonical field set, envelope integrity (the core
    authority, which rejects unsealed and tampered envelopes), object
    type, protocol and schema versions, closed state vocabulary, payload
    container type.
    """
    ...
    if envelope.protocol_version != COMPILER_PROTOCOL_VERSION:
        raise CoreValidationError(
            f"{expected_object_type} requires protocol version "
            f"{COMPILER_PROTOCOL_VERSION}, got {envelope.protocol_version!r}"
        )
    if envelope.schema_version != COMPILER_SCHEMA_VERSION:
        raise CoreValidationError(
            f"{expected_object_type} requires schema version "
            f"{COMPILER_SCHEMA_VERSION}, got {envelope.schema_version}"
        )
    try:
...
def decode_composite_json(
    ...
    *,
    ...
    if not isinstance(decoded, dict):
        raise CoreValidationError(
            f"{expected_object_type} JSON must decode to an object"
        )
...
def provenance_of(*, issuer: str, source: str, recorded_at: str) -> Provenance:
    """Deterministic provenance from declared data (no clock reads)."""
    require_utc_timestamp("provenance.recorded_at", recorded_at)
    return Provenance(issuer=issuer, source=source, recorded_at=recorded_at)
