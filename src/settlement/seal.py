"""Composite sealing for settlement-domain durable objects.

Every settlement-domain durable object is ``ObjectEnvelope + payload``
...
def decode_composite(
    ...
    *,
    ...
    rejects unsealed and tampered envelopes), object type (the two
    registry-listed settlement/finality types used exactly as registered,
    or the internal ``settlement/...`` recourse type; claiming a
    registry-governed type foreign to this record kind fails closed with
    a dedicated message), protocol and schema versions, closed state
    ...
    if envelope.object_type != object_type:
        if str(envelope.object_type).startswith("payswap/") and not object_type.startswith(
            ...
            raise CoreValidationError(
                "settlement object_type must not claim a registry-governed "
    ...
    if envelope.protocol_version != SETTLEMENT_PROTOCOL_VERSION:
        raise CoreValidationError(
            f"{object_type} requires protocol version "
            f"{SETTLEMENT_PROTOCOL_VERSION}, got {envelope.protocol_version!r}"
        )
    if envelope.schema_version != SETTLEMENT_SCHEMA_VERSION:
        raise CoreValidationError(
            f"{object_type} requires schema version "
            f"{SETTLEMENT_SCHEMA_VERSION}, got {envelope.schema_version!r}"
        )
    try:
