"""Strict fail-closed validation helpers for the settlement domain.
...
def parse_utc_timestamp(name: str, value: str) -> datetime:
    ...
    Timestamps are declared data, never clock reads: the settlement
    ...
    so ordering and window arithmetic are total, unambiguous and
    byte-stable.
    """
    require_text(name, value)
