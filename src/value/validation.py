"""Strict fail-closed validation helpers for the value domain.

Every failure raises :class:`~src.core.errors.CoreValidationError` —
the single error authority owned by ``src.core`` — with a descriptive
...
def require_identifier(name: str, value: str) -> str:
    ...
    accounts, journals, holds, funding sources and principals) use
    compact opaque tokens without whitespace so derived identifiers
    (posting and reconciliation ids built from journal identity plus a
    sequence) stay deterministic and canonical.
    """
...
def parse_timestamp(name: str, value: str) -> datetime:
    ...
    Timestamps are declared data, never wall-clock reads: the value
    domain is deterministic and never consults a clock. An explicit
    offset is required so comparisons are total and deterministic.
    """
    require_text(name, value)
    try:
