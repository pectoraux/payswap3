"""Composite sealing for safety domain durable objects.

Every safety domain durable object is ``ObjectEnvelope + payload`` and
...
from .contracts import SAFETY_PROTOCOL_VERSION, SAFETY_SCHEMA_VERSION
...
def decode_composite(
    ...
    *,
    ...
    if envelope.protocol_version != SAFETY_PROTOCOL_VERSION:
        raise CoreValidationError(
            f"{expected_object_type} requires protocol version "
            f"{SAFETY_PROTOCOL_VERSION}, got {envelope.protocol_version!r}"
        )
    if envelope.schema_version != SAFETY_SCHEMA_VERSION:
        raise CoreValidationError(
            f"{expected_object_type} requires schema version "
            f"{SAFETY_SCHEMA_VERSION}, got {envelope.schema_version}"
        )
    try:
