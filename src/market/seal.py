"""Composite sealing for market domain durable objects.

Every market domain durable object is ``ObjectEnvelope + payload`` and
...
from .contracts import MARKET_PROTOCOL_VERSION, MARKET_SCHEMA_VERSION
...
def decode_composite(
    ...
    *,
    ...
    if envelope.protocol_version != MARKET_PROTOCOL_VERSION:
        raise CoreValidationError(
            f"{expected_object_type} requires protocol version "
            f"{MARKET_PROTOCOL_VERSION}, got {envelope.protocol_version!r}"
        )
    if envelope.schema_version != MARKET_SCHEMA_VERSION:
        raise CoreValidationError(
            f"{expected_object_type} requires schema version "
            f"{MARKET_SCHEMA_VERSION}, got {envelope.schema_version}"
        )
    try:
