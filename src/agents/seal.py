"""Composite sealing for agents-domain durable objects.

Every agents-domain durable object is ``ObjectEnvelope + payload`` and
...
sealed envelope plus the canonical payload. This mirrors the sibling
pattern in ``src.safety`` and ``src.evidence``, so tampered or spliced
...
from .contracts import AGENTS_PROTOCOL_VERSION, AGENTS_SCHEMA_VERSION
...
def decode_composite(
    ...
    *,
    ...
    if envelope.protocol_version != AGENTS_PROTOCOL_VERSION:
        raise CoreValidationError(
            f"{expected_object_type} requires protocol version "
            f"{AGENTS_PROTOCOL_VERSION}, got {envelope.protocol_version!r}"
        )
    if envelope.schema_version != AGENTS_SCHEMA_VERSION:
        raise CoreValidationError(
            f"{expected_object_type} requires schema version "
            f"{AGENTS_SCHEMA_VERSION}, got {envelope.schema_version!r}"
        )
    try:
