"""Strict fail-closed validation helpers for the evidence domain.
...
def parse_utc_timestamp(name: str, value: str) -> datetime:
    ...
    Timestamps are declared data, never clock reads: the evidence domain
    is deterministic and freshness is computed only from explicit
    ``as_of`` instants and declared windows, never from the wall clock.
    The canonical ``Z`` form is required (fail closed on other offsets)
    so ordering, staleness and window arithmetic are total, unambiguous
    and byte-stable.
    """
    require_text(name, value)
...
def require_utc_timestamp_strictly_after(
    ...
    """Require ``value_b`` strictly later than ``value_a``.

    Half-open windows ``[a, b)`` are non-empty exactly when ``a < b``;
    equality is an empty window and fails closed.
    """
    if parse_utc_timestamp(name_a, value_a) >= parse_utc_timestamp(name_b, value_b):
