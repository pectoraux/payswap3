"""Composite sealing for evidence-domain durable objects.

Every evidence-domain durable object is ``ObjectEnvelope + payload`` and
...
from .contracts import EVIDENCE_PROTOCOL_VERSION, EVIDENCE_SCHEMA_VERSION
...
def decode_composite(
    ...
    *,
    ...
    if envelope.protocol_version != EVIDENCE_PROTOCOL_VERSION:
        raise CoreValidationError(
            f"{expected_object_type} requires protocol version "
            f"{EVIDENCE_PROTOCOL_VERSION}, got {envelope.protocol_version!r}"
        )
    if envelope.schema_version != EVIDENCE_SCHEMA_VERSION:
        raise CoreValidationError(
            f"{expected_object_type} requires schema version "
            f"{EVIDENCE_SCHEMA_VERSION}, got {envelope.schema_version}"
        )
    try:
