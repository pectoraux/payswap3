"""Composite sealing for intent domain durable objects.

Every intent domain durable object is ``ObjectEnvelope + payload`` and
...
sealed envelope plus the canonical payload. This mirrors the remediated
core pattern (W032: ``from_dict``/``from_json`` verify integrity and reject
unsealed or tampered envelopes) at the composite layer, so tampered or
spliced composite objects fail closed on the trusted deserialization path.
No second hash or error authority is introduced.
"""
...
from .contracts import INTENT_PROTOCOL_VERSION, INTENT_SCHEMA_VERSION
    ...
    *,
    ...
    if envelope.protocol_version != INTENT_PROTOCOL_VERSION:
        raise CoreValidationError(
            ...
            f"{INTENT_PROTOCOL_VERSION}, got {envelope.protocol_version!r}"
        )
    if envelope.schema_version != INTENT_SCHEMA_VERSION:
        raise CoreValidationError(
            f"{expected_object_type} requires schema version "
            f"{INTENT_SCHEMA_VERSION}, got {envelope.schema_version}"
        )
    try:
